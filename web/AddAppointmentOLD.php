...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<?php
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			$eSelType = test_input($_POST['selType']);
			...
			*/
			...
			if ($eSelType==1)//IF CLASSTIME
				if (isset($_POST['cbAllDay'])){$eType=1; $eTimeEnd = "23:59:00";}
				else $eType=2;
			else if ($eSelType==2)//IF OFFICEHOURS
				if (isset($_POST['cbAllDay'])){$eType=3; $eTimeEnd = "23:59:00";}
				else $eType=4;
			else if ($eSelType==3)//IF RESEARCH
				if (isset($_POST['cbAllDay'])){$eType=5; $eTimeEnd = "23:59:00";}
				else $eType=6;
			else if ($eSelType==4)//IF OTHER
				if (isset($_POST['cbAllDay'])){$eType=7; $eTimeEnd = "23:59:00";}
				else $eType=8;
		}	
